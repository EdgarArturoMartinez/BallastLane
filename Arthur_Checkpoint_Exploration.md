...
CREATE TABLE Tasks (
    ...
    Status INT NOT NULL DEFAULT 0,  -- Enum: Todo=0, InProgress=1, Done=2
...
1. docker-compose up --build
   │
   ├── [sqlserver container] starts SQL Server 2022
   │   └── Listens on port 1433 (empty, no databases yet)
   │
   ├── [api container] starts .NET API
   │   └── Program.cs calls DatabaseInitializer.InitializeAsync()
   │       │
   │       ├── Step 1: Connect to SQL Server (master db)
   │       │   └── Retry loop with exponential backoff
   │       │       (SQL Server takes ~10-15 sec to accept connections)
   │       │
   │       ├── Step 2: Execute 001_CreateDatabase.sql
...
### Discussion 7: Decisions Log — What the AI Proposed vs. What the Human Decided
...
*This checkpoint represents the exploration and analysis phase. No code has been written. The next phase will be implementation following Architecture 4 (Hexagonal / Ports & Adapters).*
